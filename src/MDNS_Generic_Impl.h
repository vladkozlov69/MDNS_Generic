...
#define __MDNS_GENERIC_IMPL_H__
...
int MDNS::_initQuery(uint8_t idx, const char* name, unsigned long timeout)
{
      ...
      (void*)this->_serviceFoundCallback)) 
  {
    ...
    statusCode = (MDNSSuccess == this->_sendMDNSMessage(0, 0,
                 (idx == 0) ? MDNSPacketTypeNameQuery : MDNSPacketTypeServiceQuery, 0));
  } 
  else
    my_free((void*)name);
...
MDNSError_t MDNS::_processMDNSQuery()
{
  ...
  if (NULL == udpBuffer) 
  {
    ...
    MDNS_LOGINFO("::_processMDNSQuery: MDNSOutOfMemory");
      ...
      else if (i >= qCnt) 
      {
        ...
        if (i < qCnt + aCnt) 
        {
          for (j = 0; j < 2; j++) 
          {
            ...
            if (servNames[j] && ((servMatches[j] && 0 == servLens[j]) || (partMatched[j] && lastWasCompressed[j]) ||
                 (servWasCompressed[j] && servMatches[j]))) 
            { 
              // somewhat handle compression by guessing
              if (buf[0] == 0 && buf[1] == ((0 == j) ? 0x01 : 0x0c) && (buf[2] == 0x00 || buf[2] == 0x80) && buf[3] == 0x01) 
              {
                recordsFound[j] = 1;
        ...
        else if (i >= (unsigned int)(qCnt + aCnt + aaCnt)) 
        {
          ...
          if (buf[1] == 0x21) 
          {
            for (j = 0; j < MDNS_MAX_SERVICES_PER_PACKET; j++) 
            {
              if (ptrNames[j] && ((firstNamePtrByte && firstNamePtrByte == ptrOffsets[j]) ||
      ...
      *p = '\0';
      ...
      for (i = 0; i < MDNS_MAX_SERVICES_PER_PACKET; i++)
      {
        if (ptrNames[i]) 
        {
          ...
          {
            this->_serviceFoundCallback(typeName, this->_resolveServiceProto, (const char*)ptrNames[i],
                                        IPAddress((const byte*)ipAddr), (unsigned short)ptrPorts[i], (const char*)servTxt[i]);
          }
        }
        ...
        else if (i == 1) 
        {
          if (this->_serviceFoundCallback) 
          {
            ...
            *p = '\0';
            ...
            this->_serviceFoundCallback(typeName, this->_resolveServiceProto, NULL, IPAddress(), 0, NULL);
          }
        }
...
{
#if ( defined(CORE_TEENSY) && ( defined(__AVR_AT90USB1286__) || defined(__AVR_ATmega32U4__) ) )
  // For Teensy++ 2.0 and Teensy 2.0 only (8-bit Atmel)
...
// 1 on success
// 0 otherwise
int MDNS::addServiceRecord(const char* name, uint16_t port, MDNSServiceProtocol_t proto, const char* textContent)
{
  int i, status = 0;
                               ...
                               MDNSServiceProtocol_t proto)
{
  ...
  for (i = 0; i < NumMDNSServiceRecords; i++)
  {
    if (port == this->_serviceRecords[i]->port && proto == this->_serviceRecords[i]->proto &&
